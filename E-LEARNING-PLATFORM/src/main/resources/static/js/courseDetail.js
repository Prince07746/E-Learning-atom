...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const lessons = JSON.parse(courseData.dataset.lessons || "[]");
    const course = courseData.dataset.course;
    const isSubscribed = courseData.dataset.subscribed === "true";

    console.log(isSubscribed);

    // Fetch and apply the saved completion statuses
// Fetch and apply the saved completion statuses
    function fetchDataLesson(isSubscribed) {
        if (isSubscribed) {
    ...
    // call the function to fetch dta
    fetchDataLesson(isSubscribed);


    const videoPlayer = document.getElementById("videos-to-play");
    ...
    mainVideo.addEventListener("timeupdate", function () {
        ...
        if (percentage > 70 && video.dataset.completed !== "true") {
    ...
    function markLessonComplete(video) {
        if(isSubscribed) {
            ...
            if (video.dataset.completed !== "true") {
                ...
                fetch('/lesson/update-completion', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({lesson: lessonId, course: courseId})
                })
